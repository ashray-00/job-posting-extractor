...
PROMPTS: dict[str, str] = {
    "v1": (
        ...
        "- required_skills: only technologies/tools that appear as contiguous "
        "phrases (or clear aliases like JS→JavaScript) in the text. Do not invent "
        "abstract paraphrases (e.g. do not write 'API design' unless those words appear).\n"
        "- nice_to_have_skills: same grounding rule; only optional/preferred skills "
        "attested in the text. Prefer [] over paraphrased filler.\n"
    ...
    "v2": (
        ...
        "extract all fields into the JSON schema provided. Rules:\n"
        "- Output ONLY valid JSON matching the schema.\n"
        "- Use null for any field not stated in the text. Prefer null / [] over guesses.\n"
        "- Use [] for list fields when nothing is stated.\n"
        "- For salary, convert to integers. Parse German formats (65.000) and "
        "abbreviations (65k). If only one number is given, set both min and max. "
        "If no salary number appears, salary_min/salary_max/salary_period must be null.\n"
        "- currency must be a 3-letter uppercase ISO 4217 code (EUR, USD, GBP), or null.\n"
        "- location_country: ISO 3166-1 alpha-2 only if the country is stated; else null.\n"
        "- location_city: ONLY if that city name appears in the posting text; else null. "
        "Never fill city from metadata or employer HQ assumptions.\n"
        "- required_skills: copy technology/tool names that literally appear in the text "
        "(minor aliasing OK: JS→JavaScript, React.js→React). Do NOT invent duty paraphrases "
        "like 'API design', 'candidate evaluation', 'sales pipeline management', "
        "'functional testing' unless those exact words appear. Prefer a short list of "
        "named tools over long soft-skill phrases. If unsure, omit.\n"
        "- nice_to_have_skills: same literal-attestation rule. Prefer [] to filler.\n"
        "- years_experience_min: ONLY if a numeric or spelled experience requirement "
        "appears (e.g. '3+ years', '3-5 years', 'mindestens 5 Jahre'). For ranges, "
        "use the lower bound. Else null.\n"
        "- remote_policy: use remote/hybrid ONLY if the text contains clear cues "
        "(remote, hybrid, home office, work from home, WFH, ortsunabhängig, etc.). "
        "Otherwise onsite or null — never invent hybrid.\n"
        "- languages: ONLY languages explicitly required in the text, with ISO 639-1 "
        "codes and CEFR levels (A1-C2) or 'native' when stated.\n"
        "- Do NOT hallucinate information not present in the text.\n\n"
        "Job posting:\n{text}"
    ),
}
